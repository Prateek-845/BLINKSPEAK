...
# PAUSE LOGIC
CHAR_PAUSE_THRESHOLD = 4.0  
WORD_PAUSE_THRESHOLD = 6.0  
...
def main():
    ...
    while cap.isOpened():
        ...
        if not is_eye_closed and current_morse != "" and time_since_open > CHAR_PAUSE_THRESHOLD:
            ...
            if char == '[BACKSPACE]': decoded_message = decoded_message[:-1]
            elif char == '[CLEAR]': decoded_message = ""
            elif char != '?': decoded_message += char
            current_morse = ""
        
        # Word Space Logic
        if not is_eye_closed and time_since_open > WORD_PAUSE_THRESHOLD and len(decoded_message) > 0 and not decoded_message.endswith(" "):
            decoded_message += " "

        #  UI DISPLAY 
        cv2.rectangle(frame, (0, frame_h - 180), (frame_w, frame_h), (20, 20, 20), -1)
        state_color = (0, 0, 255) if "CLOSED" in eye_state_text else (0, 255, 0)
        
        feedback_color = (128, 128, 128) # Gray for Ignoring
        if "DOT" in live_feedback: feedback_color = (0, 255, 255) # Yellow for Dot
        if "DASH" in live_feedback: feedback_color = (0, 165, 255) # Orange for Dash
        
        cv2.putText(frame, f"{eye_state_text}", (20, frame_h - 130), cv2.FONT_HERSHEY_SIMPLEX, 0.8, state_color, 2)
        cv2.putText(frame, f"{live_feedback}", (150, frame_h - 130), cv2.FONT_HERSHEY_SIMPLEX, 1, feedback_color, 3)
        cv2.putText(frame, f"Input: {current_morse}", (20, frame_h - 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
        
        # Final Decoded Text
        cv2.putText(frame, f"Msg: {decoded_message}", (20, frame_h - 30), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
